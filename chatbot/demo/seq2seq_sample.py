...
import numpy as np
import pandas as pd
from net.seq2seq_simple import seq2seq_simple
...
model_seq2seq_simple = seq2seq_simple(input_dic_len=len(tokenizer_ask.word_index),
                                      input_len=ask_maxlen,
                                      vector_len=20,
                                      output_dim=answer_categorical.shape[2],
                                      output_len=output_len)

model_seq2seq_simple.fit(x=ask_new, y=answer_categorical, batch_size=128, epochs=10, validation_split=0.2)
model_seq2seq_simple.save('./demo/models/seq2seq_simple_epoch_20.h5')#准确率0.70，实际表现很差
